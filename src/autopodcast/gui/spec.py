...
_SAKHA = ModeSpec(
    ...
    fields=[
        ...
        Field("--audio-clean-mode", "Режим очистки утечек", "choice", default="strict",
              advanced=True, choices=("strict", "balanced", "legacy"),
              hint="Как программа решает, чей это голос, когда микрофоны слышат друг друга.\n\nstrict → агрессивно вычищает утечку; если один микрофон громче остальных, может приписать ему чужую речь\nbalanced → мягче, меньше ошибочных переназначений\nlegacy → простое сравнение громкости, без анализа источника. Выбирай, если strict путает спикеров (открывает не тот микрофон)."),
    ],
)
