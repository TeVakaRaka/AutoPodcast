...
_SAKHA = ModeSpec(
    ...
    fields=[
        ...
        Field("--audio-clean-mode", "Режим очистки утечек", "choice", default="studio",
              ...
              hint="Как программа решает, чей это голос, когда микрофоны слышат друг друга.\n\nstudio → по умолчанию, рекомендуется. Тот же выбор микрофона, что и calibrated (громкость с калибровкой каждого канала под свой уровень), но дополнительно полностью убирает дёрганье дорожек.\ncalibrated → сравнение по громкости с калибровкой. Не путается из-за перекрученных микрофонов, но дорожки могут дёргаться.\nstrict → анализ тайминга сигнала; иногда приписывает речь не тому каналу\nbalanced → strict, но мягче\nlegacy → старое простое сравнение громкости (дёргается)"),
    ],
)
