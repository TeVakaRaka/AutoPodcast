...
_SAKHA = ModeSpec(
    ...
    fields=[
        ...
        Field("--audio-clean-mode", "Режим очистки утечек", "choice", default="calibrated",
              advanced=True, choices=("calibrated", "strict", "balanced", "legacy"),
              hint="Как программа решает, чей это голос, когда микрофоны слышат друг друга.\n\ncalibrated → по умолчанию. Сравнение по громкости с калибровкой каждого микрофона под свой уровень + защита от дёрганья. Не путается из-за перекрученных микрофонов.\nstrict → анализ тайминга сигнала; иногда приписывает речь не тому каналу\nbalanced → strict, но мягче\nlegacy → старое простое сравнение громкости (дёргается)"),
    ],
)
